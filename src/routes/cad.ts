...
router.post('/', [
  ...
  body('funcao').isString().notEmpty(),
  body('ativo').isBoolean()
], validate, (req: Request, res: Response) => {
  const { id, nome, funcao, ativo } = req.body;
  db.query('INSERT INTO cadastro (id, nome, funcao, ativo) VALUES (?, ?, ?, ?)', [id, nome, funcao, ativo], (err, results) => {
    if (err) {
...
router.put('/:id', [
  ...
  db.query('UPDATE cadastro SET nome = ?, funcao = ?, ativo = ? WHERE id = ?', [nome, funcao, ativo, id], (err) => {
    if (err) {
